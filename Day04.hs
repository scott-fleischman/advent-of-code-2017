...
main = do
  let isValid w = let ws = words w in length (List.nub ws) == length ws
  let linesAreValid = length . filter isValid . filter (not . List.null) . lines
  let test f w = putStrLn $ (if f w then "✓" else "✘") ++ " " ++ w
  let isValidB w = let ws = fmap List.sort (words w) in length (List.nub ws) == length ws
  let linesAreValidB = length . filter isValidB . filter (not . List.null) . lines
  test isValid "aa bb cc dd ee"
  test isValid "aa bb cc dd aa"
  test isValid "aa bb cc dd aaa"
  print $ linesAreValid input
  test isValidB "abcde fghij"
  test isValidB "abcde xyz ecdab"
  test isValidB "a ab abc abd abf abj"
  test isValidB "iiii oiii ooii oooi oooo"
  test isValidB "oiii ioii iioi iiio"
  print $ linesAreValidB input
